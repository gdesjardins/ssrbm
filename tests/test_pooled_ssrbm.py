...
Wh = sparse_masks.SparsityMask.unfactored_g(nh,nh,bws,bws).mask
...
def quick_alloc():
    r = pooled_ss_rbm.PooledSpikeSlabRBM(n_h=nh, n_v=nv, bw_s=bws,
            iscales={'Wv': 0.1, 'vbias':0., 'hbias':0., 'alpha':1., 'mu':0., 'beta':1.},
...
def test_energy():
    ...
    for t in xrange(batch_size):
        ...
        for j in xrange(nh):
            for si in xrange(j*bws, (j+1)*bws):
                for k in xrange(nv):
